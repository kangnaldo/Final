...
<body>
	<div class="main2">
		...
		<table class="table">
			...
			<c:if test="${!empty mySellList}">
					<c:forEach var="list" items="${mySellList }">
						<tr>
							<td>${list.id }</td>
							...
							<c:if test="${list.complete == '거래중' }">
								<td><button class="btn btn-success  btn-xs" onclick="javascript:location.href='/authority/sellResult?sellProductNo=${list.sellProductNo}&tenderId=${list.tenderPerson }'" >${list.complete }</button></td>
							</c:if>
						</tr>
					</c:forEach>
			</c:if>
